...
\begin{table}[t]
\centering
\caption{Cross-benchmark evaluation (500 samples each). SFA is trained only on ChartQA; all other benchmarks are zero-shot transfer. Performance is maintained across diverse document-centric tasks.}
\label{tab:cross}
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Benchmark} & \textbf{Metric} & \textbf{Baseline} & \textbf{+SFA} & \textbf{$\Delta$} \\
\midrule
ChartQA (2,500) & Relaxed Acc & 62.0\% & \textbf{62.9\%} & +0.9 \\
DocVQA & ANLS & 53.6\% & 52.9\% & $-0.7$ \\
InfographicVQA & ANLS & 38.7\% & 38.5\% & $-0.2$ \\
DVQA & Exact Match & 41.0\% & 40.8\% & $-0.2$ \\
FigureQA & Exact Match & 95.6\% & 95.6\% & $\pm$0.0 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:cross} evaluates generalization to benchmarks unseen during SFA training.
On the training domain (ChartQA), SFA achieves consistent improvement (+0.9\%p accuracy, $-5.5$\%p hallucination).
On zero-shot transfer benchmarks, performance remains within $\pm 1$\%p of the baseline: DocVQA ($-0.7$\%p), InfographicVQA ($-0.2$\%p), DVQA ($-0.2$\%p), and FigureQA ($\pm 0.0$\%p).
This confirms that the structural bias does not degrade general document understanding capabilities---the frozen backbone strategy prevents catastrophic forgetting across task domains.
...
\textbf{Limitations.}
The improvement magnitude is modest ($+0.9$\%p accuracy, $-5.5$\%p hallucination on ChartQA), and SFA provides domain-specific gains rather than universal improvement across all benchmarks (Table~\ref{tab:cross}).
The entropy regularization effect is small, likely because the 7,296 SFA parameters have limited capacity to reshape attention distributions dominated by the frozen 337M-parameter backbone.
Cross-architecture validation on other vision encoders (SigLIP, CLIP) remains for future work.

\textbf{Future directions.}
Promising extensions include: (1) applying SFA to SigLIP-based models (Qwen2.5-VL) and CLIP-based models (LLaVA) to validate architecture-agnostic claims; (2) training SFA on a diverse mixture of document benchmarks rather than ChartQA alone to improve cross-benchmark transfer; (3) exploring stronger regularization objectives such as numeric grounding loss with bounding-box supervision; and (4) unfreezing SFA parameters during full instruction tuning as a complementary fine-tuning strategy.
