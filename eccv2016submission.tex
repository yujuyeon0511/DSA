...
Built upon InternVL3.5-8B with a frozen backbone strategy, SFA reduces numeric hallucination from 25.5\% to 20.0\% on ChartQA while improving accuracy from 62.0\% to 62.9\%, demonstrating that minimal structural inductive bias at the encoder level can meaningfully improve document grounding.
...
\item We demonstrate that with only 7,296 trainable parameters, SFA reduces numeric hallucination by 25.5\% (from 51 to 38 cases per 200 samples) on ChartQA while improving accuracy, validating the effectiveness of minimal structural bias.
\end{itemize}
...
\textbf{Benchmark.} ChartQA~\cite{masry2022chartqa} test set (2,500 samples), evaluated using Relaxed Accuracy (correct if prediction is within 5\% of the ground truth for numeric answers, or exact match for categorical answers).
...
\textbf{Limitations.}
Our evaluation is limited to ChartQA, and the improvement magnitude is modest ($+0.9$\%p accuracy, $-5.5$\%p hallucination).
The entropy regularization effect is small, likely because the 7,296 SFA parameters have limited capacity to reshape attention distributions that are dominated by the frozen 337M-parameter backbone.
Cross-architecture validation on other vision encoders (SigLIP, CLIP) remains for future work.

\textbf{Future directions.}
Promising extensions include: (1) applying SFA to SigLIP-based models (Qwen2.5-VL) and CLIP-based models (LLaVA) to validate architecture-agnostic claims; (2) evaluating on additional benchmarks (DocVQA, TextVQA, OCRBench); (3) exploring stronger regularization objectives such as numeric grounding loss with bounding-box supervision; and (4) unfreezing SFA parameters during full instruction tuning as a complementary fine-tuning strategy.
